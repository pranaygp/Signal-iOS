...
class ProvisioningQRCodeViewController: ProvisioningBaseViewController, ProvisioningSocketManagerUIDelegate {
    ...
    private func copyCodeForAnotherScreen() async -> Bool {
        ...
        // The socket has 90s left; the primary's link message must land
        // while this process is still running in the background.
        BackgroundKeepAlive.shared.hold(reason: "link code", seconds: 95)
        return true
    }
        ...
        VStack(spacing: 8) {
            ...
            Text(LocalizationNotNeeded(
                copyState == .failed
                    ...
                    : "Linking from Signal on this same phone? Copy the code, paste it on your Mac (Preview › File › New from Clipboard), then in Signal go to Linked Devices and scan the Mac's screen. Have that scanner open before you copy. If you transfer history, stay in Signal until it finishes uploading, then come back here.",
            ))
            .font(.footnote)
